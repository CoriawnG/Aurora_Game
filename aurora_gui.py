...
def bundled_path(*relative_parts):
    """Return a bundled resource path that works in source and PyInstaller builds."""
...
def ensure_tk():
    """Load tkinter only when the legacy Tk UI is actually launched."""
    global tk, PhotoImage
    if tk is None or PhotoImage is None:
...
class AuroraGame:
    def __init__(self):
        ...
        self.advisors = {
            "economy": {
                "name": "Ada",
                "trust": 50,
                "portrait": bundled_path('assets', 'images', 'advisor_economy.png'),
            ...
            "rights": {
                "name": "Leo",
                "trust": 50,
                "portrait": bundled_path('assets', 'images', 'advisor_rights.png'),
            ...
            "security": {
                "name": "Kai",
                "trust": 50,
                "portrait": bundled_path('assets', 'images', 'advisor_security.png'),
    ...
    def _override_events_from_debates(self):
        ...
        """
        try:
            base = bundled_path('assets', 'debates')
        except Exception:
...
class AuroraGUI:
    ...
    def show_sources(self):
        ...
        # Try reading bundled docs first, then fall back to the working directory.
        content = None
        for fname in (
            bundled_path("Sources.txt"),
            bundled_path("README.md"),
            "SOURCES.txt",
            "CITATIONS.md",
            "SOURCES.md",
        ):
